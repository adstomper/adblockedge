...
let optionsObserver =
{
  ...
  initOptionsDoc: function(/**Document*/ doc)
  {
    ...
    function initCheckboxes()
    {
      ...
      if (window)
      {
        setChecked("adblockedge-showintoolbar", UI.isToolbarIconVisible(window));
...
let UI = exports.UI =
{
  ...
  onToolbarCustomization: function(/**Event*/ event)
  {
    ...
    if (this.isToolbarIconVisible(icon))
    {
  ...
  {
    let visible = "visible", parent = null, before;
    if (position)
    {
      [visible, parent, before] = position.split(",", 3);
      parent = toolbox.ownerDocument.getElementById(parent);
      if (parent && parent.hasAttribute("toolbar-delegate"))
      {
        // Ignore delegated toolbars to avoid hitting
   ...
   */
  isToolbarIconVisible: function(/**Window|Element*/ window)
  {
    let icon = (window instanceof Ci.nsIDOMWindow ?
                window.document.getElementById("abp-toolbarbutton") :
                window);
    if (!icon)
      return false;

    for (let parent = icon.parentNode; parent; parent = parent.parentNode)
