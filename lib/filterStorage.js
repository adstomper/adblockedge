...
let FilterStorage = exports.FilterStorage =
{
  ...
  get sourceFile()
  {
    ...
    if (!file)
    {
      ...
      try
      {
        file = IO.resolveFilePath(Services.prefs.getDefaultBranch("extensions.adblockedge.").getCharPref("data_directory"));
        if (file)
          file.append("patterns.ini");
      } catch(e) {}
    }
    ...
    if (!file)
      Cu.reportError("Adblock Plus: Failed to resolve filter file location from extensions.adblockedge.patternsfile preference");
