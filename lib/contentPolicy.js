...
let PolicyImplementation =
{
  ...
  init: function()
  {
    ...
    onShutdown.add(function()
    {
      ...
      Services.obs.removeObserver(this, "xpcom-category-entry-removed", true);
      Services.obs.removeObserver(this, "xpcom-category-cleared", true);
  ...
  observe: function(subject, topic, data, additional)
  {
    switch (topic)
    {
      ...
      {
        let category = data;
        if (!this.xpcom_categories.some(function(c) c == category))
          return;

        if (topic == "xpcom-category-entry-removed" && subject instanceof Ci.nsISupportsCString)
          if (subject.data != this.contractID)
            return;
